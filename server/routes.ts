...
import { getTwilioConnectionStatus, sendTestSMS, sendBookingConfirmationSMS, sendBookingStatusUpdateSMS, sendDriverAssignmentSMS, sendSMS, sendDriverOnTheWaySMS, sendDriverArrivedSMS, sendBookingCancelledSMS, sendAdminNewBookingAlertSMS, sendPasswordResetSMS, sendTemporaryPasswordSMS, sendUsernameReminderSMS, getGatewayStatus, setActiveGateway, testSMSConnection } from "./sms";
import type { SMSGatewayType } from "./sms";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ========================================
  // SMS Gateway Management (Multi-provider)
  // ========================================

  // Get SMS gateway status for all providers
  app.get('/api/admin/sms/gateway', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const user = await storage.getUser(userId);
      ...
      console.error('SMS gateway status error:', error);
      res.status(500).json({ message: 'Failed to get SMS gateway status' });
    }
  });

  // Set active SMS gateway
  app.post('/api/admin/sms/gateway', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const user = await storage.getUser(userId);
      ...
      if (!['twilio', 'amazon-sns', 'plivo'].includes(gateway)) {
        return res.status(400).json({ message: 'Invalid gateway type' });
      }

      await setActiveGateway(gateway as SMSGatewayType, userId);
      res.json({ success: true, activeGateway: gateway });
    } catch (error) {
      console.error('Set SMS gateway error:', error);
      res.status(500).json({ message: 'Failed to set SMS gateway' });
    }
  });

  // Save Amazon SNS credentials
  app.post('/api/admin/sms/credentials/amazon-sns', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const user = await storage.getUser(userId);
      ...
      const { accessKeyId, secretAccessKey, region, senderId } = req.body;

      if (!accessKeyId || !region) {
        return res.status(400).json({ message: 'Access Key ID and Region are required' });
      }

      await storage.setSetting('AWS_SNS_ACCESS_KEY_ID', accessKeyId, userId);
      await storage.setSetting('AWS_SNS_REGION', region, userId);
      
      if (secretAccessKey) {
        await storage.setSetting('AWS_SNS_SECRET_ACCESS_KEY', secretAccessKey, userId);
      }
      
      if (senderId !== undefined) {
        await storage.setSetting('AWS_SNS_SENDER_ID', senderId || '', userId);
      }

      res.json({ success: true, message: 'Amazon SNS credentials saved successfully' });
    } catch (error) {
      console.error('Save Amazon SNS credentials error:', error);
      res.status(500).json({ message: 'Failed to save Amazon SNS credentials' });
    }
  });

  // Get Amazon SNS credentials (masked)
  app.get('/api/admin/sms/credentials/amazon-sns', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const user = await storage.getUser(userId);
      ...
      const [accessKeyId, secretAccessKey, region, senderId] = await Promise.all([
        storage.getSetting('AWS_SNS_ACCESS_KEY_ID'),
        storage.getSetting('AWS_SNS_SECRET_ACCESS_KEY'),
        storage.getSetting('AWS_SNS_REGION'),
        storage.getSetting('AWS_SNS_SENDER_ID'),
      ]);

      res.json({
        accessKeyId: accessKeyId?.value || '',
        hasSecretAccessKey: !!secretAccessKey?.value,
        region: region?.value || 'us-east-1',
        senderId: senderId?.value || '',
      });
    } catch (error) {
      console.error('Get Amazon SNS credentials error:', error);
      res.status(500).json({ message: 'Failed to get Amazon SNS credentials' });
    }
  });

  // Save Plivo credentials
  app.post('/api/admin/sms/credentials/plivo', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const user = await storage.getUser(userId);
      ...
      if (!authId || !phoneNumber) {
        return res.status(400).json({ message: 'Auth ID and Phone Number are required' });
      }

      await storage.setSetting('PLIVO_AUTH_ID', authId, userId);
      await storage.setSetting('PLIVO_PHONE_NUMBER', phoneNumber, userId);
      
      if (authToken) {
        await storage.setSetting('PLIVO_AUTH_TOKEN', authToken, userId);
      }

      res.json({ success: true, message: 'Plivo credentials saved successfully' });
    } catch (error) {
      console.error('Save Plivo credentials error:', error);
      res.status(500).json({ message: 'Failed to save Plivo credentials' });
    }
  });

  // Get Plivo credentials (masked)
  app.get('/api/admin/sms/credentials/plivo', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const user = await storage.getUser(userId);
      ...
      const [authId, authToken, phoneNumber] = await Promise.all([
        storage.getSetting('PLIVO_AUTH_ID'),
        storage.getSetting('PLIVO_AUTH_TOKEN'),
        storage.getSetting('PLIVO_PHONE_NUMBER'),
      ]);
      ...
      console.error('Get Plivo credentials error:', error);
      res.status(500).json({ message: 'Failed to get Plivo credentials' });
    }
  });

  // Test specific SMS gateway connection
  app.post('/api/admin/sms/gateway/test', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const user = await storage.getUser(userId);
      ...
      const result = await testSMSConnection(gateway as SMSGatewayType);
      res.json(result);
    } catch (error) {
      console.error('Test SMS gateway error:', error);
      res.status(500).json({ connected: false, error: 'Failed to test SMS gateway' });
    }
  });
