...
## Recent Changes
- **November 1, 2025**: Implemented automatic booking expiration system - past-due bookings are automatically cancelled and drivers are released from expired jobs. Backend endpoint POST /api/bookings/auto-cancel-expired runs when driver dashboard loads, finding bookings where scheduledDateTime has passed and status is still active, then cancelling them and removing driver assignments. Frontend filters out past-due bookings from driver's accepted jobs view for immediate visual feedback.
