...
## Recent Changes
- **November 1, 2025**: Enhanced media deletion to properly handle all database records - when deleting a media file, the system now automatically clears the active site logo setting if the deleted file was set as the site logo, then deletes both the physical file from object storage and the database record. Includes proper 404 handling and error recovery.
