...
### November 11, 2025
- **AdminNav Integration for MinIO Browser**: Integrated AdminNav header component into MinIO Browser page for consistent navigation across all admin pages. Removed redundant "Back to Admin" button and replaced it with the comprehensive AdminNav header featuring USA Luxury Limo logo (links to /admin), "Admin Portal" title, dropdown menus (Manage, Settings, Dispatch), and logout button. Settings dropdown includes "Browse Images" menu item that navigates to /admin/minio-browser. Page structure updated to use React fragment wrapper with AdminNav at the top, maintaining all existing functionality (Upload Assets, search, pagination, image preview). Verified via e2e testing that AdminNav renders correctly, navigation works as expected, and no duplicate navigation elements remain.
- **MinIO Bucket Auto-Creation**: Implemented automatic bucket creation system that ensures MinIO buckets exist before any file operations. Features include: `ensureBucket()` method in S3StorageAdapter using AWS SDK HeadBucketCommand/CreateBucketCommand, global bucket existence cache (keyed by endpoint:bucket) to prevent repeated API calls, initPromise pattern where all 6 public methods (upload, download, delete, list, getDownloadUrl, listWithMetadata) await bucket initialization, comprehensive error handling for race conditions (409), permission errors (403), and NotFound scenarios (404), and intelligent fallback to 'usa-luxury-limo' default bucket name. Fixed critical bug where SystemSetting objects were passed instead of string values to storage adapter, causing "bucketName.split is not a function" error. System now works seamlessly on fresh MinIO installations with no pre-created buckets.
- **MinIO Dynamic Credential Loading**: Implemented database-driven MinIO credential system with intelligent fallback. The storage adapter now reads credentials from database settings (configured via admin dashboard) as the primary source, with automatic fallback to environment variables. Features include: async `getObjectStorage()` function that fetches credentials from database on first access with proper `.value` extraction from SystemSetting objects, 1-minute credential cache for performance optimization, automatic cache refresh when MinIO settings are updated via admin panel, graceful error handling when database is unavailable, and comprehensive null safety in browse endpoint to prevent crashes. All 10+ storage-dependent routes updated to properly await async initialization, preventing runtime errors. This enables flexible deployment where credentials can be managed through the web interface without requiring server environment variable changes or restarts.
- **MinIO File Upload**: Added comprehensive file upload functionality to MinIO Image Browser for testing and managing files directly in object storage. Features include: "Upload Assets" button with luxury gradient styling in browser header, upload dialog modal with folder selector (defaults to current filter), drag-and-drop support with visual feedback, file picker with browse button, client-side validation (2MB limit, JPEG/PNG/GIF/WebP/HEIC formats), loading states during upload, real-time cache invalidation to refresh file list after upload, error handling with toast notifications, and folder sanitization for security. Backend endpoint `/api/admin/minio/upload` uses `getObjectStorage()` to automatically detect and use MinIO credentials from admin dashboard, implements admin authentication guard (isAuthenticated + requireAdmin), uses multer for file handling, prevents path traversal attacks, and returns file metadata in same format as browse endpoint for seamless integration.
